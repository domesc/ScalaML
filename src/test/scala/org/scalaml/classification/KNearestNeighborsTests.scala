...
class KNearestNeighborsTests extends FlatSpec with Matchers {
  ...
  "findTopK" should "find the top 2 classes with smaller distance" in {
    ...
    val features = DenseMatrix(row1, row2, row3, row4)
    val labels: DenseVector[Double] = DenseVector(1.0, 2.0, 1.0, 3.0)
  ...
  "predict" should "classify well" in {
    ...
    val features = DenseMatrix(row1, row2, row3, row4, row5)
    val labels: DenseVector[Double] = DenseVector(1.0, 2.0, 1.0, 3.0, 2.0)
    val model = SimpleNearestNeighbors(features, labels, k = 3)
    val testSet: DenseMatrix[Double] = DenseMatrix((1.2, 2.3, 5.2), (8.9, 1.2, 40.0))
    val predictions = model.predict(testSet)

    predictions shouldEqual DenseVector(2.0, 2.0)
  }
